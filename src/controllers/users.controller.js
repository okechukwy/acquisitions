...
export const updateUserById = async (req, res, next) => {
    try {
        ...
        if (currentUserRole !== 'admin' && currentUserId !== id) {
            logger.warn(`User ${currentUserId} attempted to update user ${id} without permission`);
        ...
        if (currentUserRole !== 'admin' && validatedData.role) {
            logger.warn(`User ${currentUserId} attempted to change role without admin privileges`);
            return res.status(403).json({
...
export const deleteUserById = async (req, res, next) => {
    try {
        ...
        if (currentUserRole !== 'admin') {
            logger.warn(`User ${req.user?.id} attempted to delete user ${id} without admin privileges`);
            return res.status(403).json({

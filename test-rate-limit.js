...
async function testRateLimit(endpoint, maxRequests = 10, delayMs = 100) {
  ...
  console.log(`Making ${maxRequests} requests with ${delayMs}ms delay between requests...\n`);
  ...
  const successful = results.filter(r => r.status < 400 && r.status !== 'ERROR').length;
...
async function main() {
  ...
  console.log('\n💡 Expected behavior:');
  console.log('   - General endpoints: 100 requests/minute (should allow most)');
  console.log('   - Auth endpoints: 5 requests/minute (should block after 5-10)');
  console.log('   - Signup endpoint: 3 requests/minute (should block after 3-5)');
}
